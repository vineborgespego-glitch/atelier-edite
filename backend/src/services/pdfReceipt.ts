  ...
  return new Promise((resolve, reject) => {
    try {
      ...
      order.items.forEach(item => {
        ...
        // Print quantity and description with wrapping
        doc.text(`${qty}x ${item.description}`, { width: 130 });
        const endDescriptionY = doc.y;
        
        // Print subtotal right-aligned on the first line of this item
        doc.text(`R$ ${subtotal}`, 15, startY, { align: 'right', width: 197 });
        
        // Move to the maximum Y reached plus a gap (spacing between items)
        doc.y = Math.max(endDescriptionY, doc.y) + 10;
      });
      doc.moveDown(0.5);
      doc.text('----------------------------------');
      ...
      // --- TOTAL ---
      doc.moveDown(0.5);
      doc.fontSize(14).font('Courier-Bold').text('TOTAL:', { continued: true });
