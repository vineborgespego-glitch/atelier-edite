...
export async function sendVerificationEmail(to: string, code: string, userName: string) {
  const mailOptions = {
    from: `"Atelier Edite" <${process.env.SMTP_USER}>`,
    to,
    subject: `${code} é o seu código de verificação — Atelier Edite`,
    html: `
      <div style="font-family: sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; border: 1px solid #f5e6e8; border-radius: 10px;">
        <h2 style="color: #b5777b; text-align: center;">Bem-vinda ao Atelier Edite!</h2>
        ...
        <p style="text-align: center; font-size: 12px; color: #999;">
          Atelier Edite — A gestão que entende a arte da costura.<br/>
          São Paulo, Brasil
        </p>
      </div>
    `,
